...
BLOB_NAME = "myfolder/files/md5/07/default_of_credit_card_clients.csv"

# Helper function to download the dataset from Azure Blob Storage
def download_blob_to_df(container_name, blob_name):
    try:
        ...
        df = pd.read_csv(StringIO(data_str), skiprows=1, index_col=0)
        return df
    except Exception as e:
        st.error(f"Error downloading data from Azure: {e}")
        return None
...
if st.session_state.jwt_token:
    ...
    # Input fields for the 23 features (you can add more as needed)
    ...
    df = download_blob_to_df(CONTAINER_NAME, BLOB_NAME)
